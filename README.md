...
3. Build and start containers:
   ...
   docker-compose up
   ```
Here is the corrected Markdown, including the explanation section in proper formatting:
...
- **JSON + Base64 Response**: 
  - For each uploaded image, the AI service returns a JSON object containing detections and a base64-encoded annotated image with bounding boxes.
- **Dockerized**: 
  - Each service has its own Dockerfile, orchestrated by `docker-compose`.
- **Progress Bar**: 
  - Users can track the progress of their image upload from the browser.
- **Automatic Bounding Box Display**: 
  - The UI decodes the base64-encoded annotated image and shows it directly in the browser, along with detection data.
...
3. **UI Service** receives the file at `/upload` and forwards it to the AI service (`POST /detect`).  
4. **AI Service** uses Ultralytics YOLOv8 to run inference:
   - Detects objects, draws bounding boxes, and encodes the result to base64.
   - Returns a JSON response:
     ...
     ```
5. **UI Service** passes the JSON response back to the browser.  
6. **Browser** displays the JSON detection output and the annotated bounding-box image.
...
Troubleshooting
	...
	•	If YOLOv8 weights fail to download, place them manually or ensure internet connectivity.
	•	Performance:
	•	YOLOv8n on CPU might be slow for large images. Use smaller images or upgrade to GPU-based Docker if available.
	•	Memory Errors:
	•	Large images or low system RAM might cause issues. Resize or optimize images.
